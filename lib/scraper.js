...
import { db, twSearch, yt, igr, twStream } from "./db";
import "../lib/cron";
...
// Twitter Stream
export async function streamTwitter(keyword) {
  ...
  stream.on("data", function(item) {
    ...
    const twstream = twStream
      ...
      .uniqBy("id")
      .value();
    twStream.set("twitter", twstream).write();
    return item;
  });
}
...
export function algoliaTwitter() {
  ...
  const stream = require("../dbTStream.json");
  index.addObjects(stream.twitter);
}
...
export async function runStream() {
  streamTwitter("jokowi");
  console.log("Run Scrap Stream");
}
